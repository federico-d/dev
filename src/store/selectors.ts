...
import { computeBaseDamageScenarios, computeDamageCategories, computeDamageScenarios } from '../domain/engines/damage-engine';
import { computeAttackStepInstances } from '../domain/engines/activation-engine';
import { buildApplicableRiskSources, buildPreliminaryAttackPaths, computeRiskRows } from '../domain/engines/risk-engine';
...
export function selectApplicableDamageScenariosByAttackStep(state: AnalysisStoreState) {
  const attackStepInstances = selectAttackStepInstances(state);
  const damageScenarios = selectDamageScenarios(state);
  return buildApplicableRiskSources(attackStepInstances, damageScenarios);
}

export function selectAttackPaths(state: AnalysisStoreState) {
  const attackStepInstances = selectAttackStepInstances(state);
  ...
  return links.flatMap((link) => {
    ...
    if (!step) return [];
    return buildPreliminaryAttackPaths(step, link.damageScenarioIds, link.strategy);
  });
}
...
export function selectMitigationSummary(state: AnalysisStoreState) {
  ...
  return {
    ...
    highestRemainingRiskPresent: rows
      ...
      .filter(Boolean)
      .sort((a, b) => (RISK_LEVEL_RANK[a as string] ?? 0) - (RISK_LEVEL_RANK[b as string] ?? 0))
      .slice(-1)[0] ?? null,
    netRapDistribution,
...
export function selectDamageScenarios(state: AnalysisStoreState) {
  return computeDamageScenarios(
    state.questionnaire.byId,
    DAMAGE_RULES,
    ASSUMPTIONS_CATALOG.filter((item) => item.activeByTemplate),
  );
}

export function selectBaseDamageScenarios(state: AnalysisStoreState) {
  return computeBaseDamageScenarios(state.questionnaire.byId, DAMAGE_RULES);
}

export function selectDamageCategories(state: AnalysisStoreState) {
  return computeDamageCategories(
    state.questionnaire.byId,
    DAMAGE_RULES,
    ASSUMPTIONS_CATALOG.filter((item) => item.activeByTemplate),
  );
...
export function selectDamageTransformationSummary(state: AnalysisStoreState) {
  const scenarios = selectDamageScenarios(state);
  return {
    total: scenarios.length,
    transformed: scenarios.filter((item) => item.transformed).length,
    excluded: scenarios.filter((item) => item.excludedFromScope).length,
    included: scenarios.filter((item) => !item.excludedFromScope).length,
  };
}
