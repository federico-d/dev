...
export type DamageRule = {
  ...
  note: string;
  sourceSheet: 'Damage Scenarios' | 'Questions';
  workbookNotes?: string;
  tags?: string[];
};
...
export type AssumptionEffectType =
  | 'noEffect'
  | 'scopeReduction'
  | 'damageTransformation'
  | 'attackStepConstraint'
  | 'operationalConstraint'
  | 'outputOnly';

export type AssumptionEffectConfig = {
  targetDamageCategories?: DamageCategory[];
  targetDamageLevels?: DamageLevelLabel[];
  sourceQuestionIds?: string[];
  transformationNotes?: string[];
  scopeImpactNotes?: string[];
  excludedScenarioKinds?: string[];
  safetyException?: boolean;
};
...
export type AssumptionDefinition = {
  ...
  effectType: AssumptionEffectType;
  effectConfig: AssumptionEffectConfig;
  sourceSheet: 'Assumptions';
  workbookNotes?: string;
  tags?: string[];
  activeByTemplate?: boolean;
  allowedInMitigation?: boolean;
};
...
export type CountermeasureDefinition = {
  ...
  configFlags: {
    ...
    needsManualEaseValues: boolean;
    dependsOnProductCapabilities: boolean;
    affectsOutputText: boolean;
  };
...
export type AttackStepQualityFlags = {
  estimatedFromWorkbookContext?: boolean;
  partiallyInferred?: boolean;
  needsParityReview?: boolean;
  missingWorkbookLiteral?: boolean;
  unresolvedLinkingGap?: boolean;
};

export type AttackStepDamageHint = {
  sourceQuestionIds: string[];
  categories: DamageCategory[];
  scenarioLabels: string[];
  explicitDamageScenarioRefs?: string[];
  notes: string;
};

export type ActivationAdjustment = {
  questionId: string;
  kind: 'easeModifier' | 'context';
  factor?: EaseFactorName;
  delta?: number;
  note: string;
};
...
export type AttackStepDefinition = {
  ...
  linkedDamageScenarioHints: AttackStepDamageHint;
  ...
  qualityFlags?: AttackStepQualityFlags;
};
...
export type AttackStepInstance = {
  ...
  activationAdjustments?: ActivationAdjustment[];
  linkedDamageScenarioHints: AttackStepDamageHint;
  qualityFlags?: AttackStepQualityFlags;
};
...
export type AttackPathSourceStrategy =
  | 'explicitWorkbookLink'
  | 'inferredFromCategory'
  | 'inferredFromQuestionSource'
  | 'fallbackSingleStep';
...
export type AttackPath = {
  ...
  title?: string;
  pathKind: 'singleStep' | 'multiStep';
  entryPoint?: string;
  targetDamageScenarioIds: string[];
  notes: string;
  qualityFlags?: AttackStepQualityFlags;
  sourceStrategy: AttackPathSourceStrategy;
};

export type RiskRowSource = {
  strategy: 'linkedDamageScenariosOnly' | 'fallbackAllScenarios';
  note: string;
};

export type AttackStepDamageLinkResult = {
  attackStepId: string;
  damageScenarioIds: string[];
  strategy: 'explicitHints' | 'questionSource' | 'categoryFallback' | 'noMatchFallback';
  notes: string[];
...
export type RiskRow = {
  ...
  attackPathSourceStrategy: AttackPathSourceStrategy;
  attackStepId: string;
  attackStepTitle: string;
  active: boolean;
  damageScenarioId: string;
  damageScenarioLabel: string;
