...
describe('Sprint 2B pages', () => {
  ...
  it('renders Attack Steps page with summary and table', () => {
    ...
    expect(screen.getByText('Explicit linking')).toBeInTheDocument();
  ...
  it('Attack Steps page supports linking filter', () => {
    render(<AttackStepsPage />);
    fireEvent.change(screen.getByLabelText('Linking'), { target: { value: 'fallback' } });
    expect(screen.getByText('AS37')).toBeInTheDocument();
  });
});
